...
<?php
	...
	if(isset($_POST['login'])){
		...
		$query      	= mysql_query("SELECT id_member, level FROM member where nama = '$nama' AND password='$password'");
		...
		if ($numb == 1){
			...
			$_SESSION['level']	= $data['level'];
				...
				if( $_SESSION['level'] == "user"){
					echo"<script>alert('Login sukses'); document.location = 'index.php?pg=homeMember';</script>";
				}
				else if ( $_SESSION['level'] == "admin"){
					echo"<script>alert('Login sukses'); document.location = 'index.php?pg=tabelBarang';</script>";
				}
		}
					...
					<center>
						<div class="panel-body" style="width:100%;">
							<form method="post" action="?pg=login" class="form-horizontal container-fluidpelanggan">
								  ...
								  <input type="text" class="form-control" id="inputSuccess5" aria-describedby="inputSuccess5Status" name="nama" placeholder="Username">
